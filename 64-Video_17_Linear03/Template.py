...
class Video_1(FrameScene):
    def construct(self):
        ...
        matrix = MTex(r"\begin{bmatrix}1&1\\2&4\end{bmatrix}\begin{bmatrix}{x}\\y\end{bmatrix}=\begin{bmatrix}10\\28\end{bmatrix}", tex_to_color_map = color_map).scale(0.8)
...
class Video_3(FrameScene):
    def construct(self):
        def equations(*numbers: list[int]):
            ...
            mtex = MTex(r"\begin{cases}{"+str(numbers[0])+r"}x+{"+str(numbers[1])+r"}y="+str(numbers[2])+r"\\{"+str(numbers[3])+r"}x+{"+str(numbers[4])+r"}y="+str(numbers[5])+r"\end{cases}", color = ORANGE, tex_to_color_map = color_map).set_stroke(width = 4, color = BLACK, background = True)
        ...
        equations_4 = MTex(r"\begin{cases}3x+2y+1z=39\\2x+3y+1z=34\\1x+2y+3z=26\end{cases}", tex_to_color_map = {r"3x+2y+1z=39": BLUE, r"2x+3y+1z=34": TEAL, r"1x+2y+3z=26": GREEN}).scale(0.8).shift(2.5*UP + 3*RIGHT)
        ...
        cases_4 = VGroup(MTex("???", color = BLUE).scale(2).shift(2*RIGHT + 2.5*DOWN), Polyline(3.05*LEFT + 2*DOWN, 3.05*LEFT + 0.8*UP, 3.05*RIGHT + 0.8*UP, 3.05*RIGHT + 2*DOWN, color = YELLOW).shift(2*RIGHT + 2.5*DOWN))
...
class Video_4(FrameScene):
    def construct(self):
        problem = Heiti("今有上禾三秉，中禾二秉，下禾一秉，實三十九斗；\n　　上禾二秉，中禾三秉，下禾一秉，實三十四斗；\n　　上禾一秉，中禾二秉，下禾三秉，實二十六斗。\n問上、中、下禾實一秉各幾何？", t2c = {("上禾", "上"): GREEN, ("中禾", "中"): TEAL, ("下禾", "下"): BLUE, ("一秉", "二秉", "三秉"): ORANGE, ("三十九斗", "三十四斗", "二十六斗"): YELLOW}).scale(2/3).shift(2.5*UP)
        equation_rough = MTex(r"\begin{cases}3x+2y+z=39\\2x+3y+z=34\\x+2y+3z=26\end{cases}", tex_to_color_map = {r"3x+2y+z=39": BLUE, r"2x+3y+z=34": TEAL, r"x+2y+3z=26": GREEN}).scale(0.8)
        ...
        matrix = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[0]: ORANGE, texts[1]: GREEN, texts[2]: YELLOW}).scale(0.8).shift(UP)
        ...
        notation = MTex(r"A\vec{x}=\vec{b}", tex_to_color_map = {r"A": ORANGE, r"\vec{x}": GREEN, r"\vec{b}": YELLOW}).shift(2.5*UP).save_state()
        ...
        vector = MTex(r"{1}"+texs[0]+r"+{2}"+texs[1]+r"+{1}"+texs[2]+r"="+texs[3], tex_to_color_map = {(texs[0], texs[1], texs[2]): ORANGE, (r"{1}", r"{2}"): GREEN, texs[3]: YELLOW}).scale(0.8).shift(0.5*DOWN)
        ...
        calculation = MTex(texs[0]+r"{x}+"+texs[1]+r"{y}+"+texs[2]+r"{z}="+texs[0]+r"{x}+"+texs[1]+r"{y}+"+texs[2]+r"{z}", tex_to_color_map = {(texs[0], texs[1], texs[2]): ORANGE, (r"{x}", r"{y}", r"{z}"): GREEN}).scale(0.8).shift(0.5*DOWN)
        ...
        equation = MTex(r"\begin{bmatrix}3x+2y+1z\\2x+3y+1z\\1x+2y+3z\end{bmatrix}=\begin{bmatrix}8\\9\\8\end{bmatrix}", tex_to_color_map = {r"\begin{bmatrix}8\\9\\8\end{bmatrix}": YELLOW}).scale(0.8).set_y(-0.5)
        for i in [3, 6, 9, 11, 14, 17, 19, 22, 25]:
        ...
        solution = MTex(r"\begin{cases}3x+2y+1z=8\\2x+3y+1z=9\\1x+2y+3z=8\end{cases}\Rightarrow \begin{cases}x=1\\y=2\\z=1\end{cases}", tex_to_color_map = {(r"x", r"y", r"z", r"{1}", r"{2}"): GREEN, (r"1", r"2", r"3"): ORANGE, (r"8", r"9"): YELLOW}).scale(0.7).shift(2.4*DOWN)
        ...
        self.add(right, shade, left).play(left.animate.restore(), follow(shade, left, remover = True), right.animate.restore(), Flip(input, MTex(r"\begin{bmatrix}1\\2\\1\end{bmatrix}", color = GREEN).scale(0.8).shift(5.6*LEFT + 0.5*DOWN)), run_time = 2)
        self.wait()
...
class Video_5(FrameScene):
    def construct(self):
        ...
        output_2 = MTex(r"\begin{bmatrix}8\\9\\8\end{bmatrix}", color = YELLOW).scale(0.8).shift(1.5*DOWN + 3*RIGHT).save_state()
...
class Video_6(FrameScene):
    def construct(self):
        box = BlackBox().set_stroke(color = BLACK, width = 16).add(BlackBox()).add(MTex(r"\begin{bmatrix}4&2\\1&1\end{bmatrix}", color = ORANGE)).shift(2*UP)
        equation = MTex(r"\begin{cases}4x+2y=6\\1x+1y=2\end{cases}", tex_to_color_map = {("1", "2", "4"): ORANGE, ("=2", "6"): YELLOW, "=": WHITE, "x": BLUE, "y": GREEN}).shift(DOWN)
        ...
        matrix = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[0]: ORANGE, texts[1]: GREEN, texts[2]: YELLOW}).set_stroke(width = 8, color = BLACK, background = True).shift(4*LEFT + DOWN)
        parts = [matrix.get_part_by_tex(tex) for tex in [texts[0], texts[1], r"=", texts[2]]]
        ...
        label_1 = MTex(r"(1, 1)",color = GREEN).next_to(point_1, UL, buff = 0.15).set_stroke(width = 8, color = BLACK, background = True)
        label_2 = MTex(r"(6, 2)",color = YELLOW).next_to(point_2, UR, buff = 0.15).set_stroke(width = 8, color = BLACK, background = True)
...
class Video_7(FrameScene):
    def construct(self):
        box = BlackBox().set_stroke(color = BLACK, width = 16).add(BlackBox()).add(MTex(r"\begin{bmatrix}4&2\\1&1\end{bmatrix}", color = ORANGE)).shift(2*UP + 4*LEFT)
        ratio = 0.4
        ...
        label_1 = MTex(r"(1, 2)",color = GREEN).next_to(point_1, UL, buff = 0.15).set_stroke(width = 8, color = BLACK, background = True)
        label_2 = MTex(r"(8, 3)",color = YELLOW).next_to(point_2, UL, buff = 0.15).set_stroke(width = 8, color = BLACK, background = True)
        texts = r"\begin{bmatrix}4&2\\1&1\end{bmatrix}", r"\begin{bmatrix}1\\2\end{bmatrix}", r"\begin{bmatrix}8\\3\end{bmatrix}"
        matrix = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[0]: ORANGE, texts[1]: GREEN, texts[2]: YELLOW}).set_stroke(width = 8, color = BLACK, background = True).shift(4*LEFT + DOWN)
        notation = MTex(r"A\vec{x}=\vec{b}", tex_to_color_map = {r"A": ORANGE, r"\vec{x}": GREEN, r"\vec{b}": YELLOW}).set_stroke(width = 8, color = BLACK, background = True).next_to(matrix, DOWN)
        ...
        box_2 = box_wide.copy().set_stroke(color = BLACK, width = 16).add(box_wide).add(MTex(r"\begin{bmatrix}0.5&-1\\-0.5&2\end{bmatrix}", color = TEAL)).shift(2*UP + 4*LEFT)
        texts = r"\begin{bmatrix}0.5&-1\\-0.5&2\end{bmatrix}", r"\begin{bmatrix}8\\3\end{bmatrix}", r"\begin{bmatrix}1\\2\end{bmatrix}"
        matrix_2 = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[0]: TEAL, texts[1]: YELLOW, texts[2]: GREEN}).set_stroke(width = 8, color = BLACK, background = True).shift(4*RIGHT + DOWN)
        notation_2 = MTex(r"A^{-1}\vec{b}=\vec{x}", tex_to_color_map = {r"A^{-1}": TEAL, r"\vec{x}": GREEN, r"\vec{b}": YELLOW}).set_stroke(width = 8, color = BLACK, background = True).next_to(matrix_2, DOWN)
...
class Video_8(FrameScene):
    def construct(self):
        box = BlackBox().set_stroke(color = BLACK, width = 16).add(BlackBox()).add(MTex(r"\begin{bmatrix}4&2\\2&1\end{bmatrix}", color = ORANGE)).shift(1.5*UP)
        self.add(box).wait()
        ...
        label_1 = MTex(r"(6, 3)",color = YELLOW).next_to(point_1, UL, buff = 0.15).set_stroke(width = 8, color = BLACK, background = True)
        ...
        matrix_2 = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[0]: ORANGE, texts[1]: GREEN, texts[2]: YELLOW}).set_stroke(width = 8, color = BLACK, background = True).shift(4*LEFT + DOWN)
...
class Video_9(FrameScene):
    def construct(self):
        ...
        rank_left = MTexText(r"秩为2", color = RED).scale(1.2).shift(0.8*UP + 32/9*LEFT).set_stroke(width = 8, color = BLACK, background = True)
        rank_right = MTexText(r"秩为1", color = RED).scale(1.2).shift(0.8*UP + 32/9*RIGHT).set_stroke(width = 8, color = BLACK, background = True)
...
class Video_10(FrameScene):
    def construct(self):
        ...
        equation = MTex(r"\begin{bmatrix}4\\2\end{bmatrix}=2\begin{bmatrix}2\\1\end{bmatrix}", tex_to_color_map = {r"\begin{bmatrix}2\\1\end{bmatrix}": GREEN, r"\begin{bmatrix}4\\2\end{bmatrix}": BLUE}).scale(0.8).shift(0.5*RIGHT + DOWN).set_stroke(width = 16, color = BLACK, background = True)
        ...
        matrix = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[1]: TEAL, texts[2]: GREY}).set_stroke(width = 16, color = BLACK, background = True).shift(4*LEFT + 0.5*DOWN)
        for i in range(6):
        ...
        matrix_2 = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[1]: TEAL, texts[2]: GREY}).set_stroke(width = 16, color = BLACK, background = True).shift(4*LEFT + 0.5*DOWN)
        for i in range(6):
        ...
        function = MTex(r"\begin{bmatrix}1\\0.5\end{bmatrix}+k\begin{bmatrix}-1\\2\end{bmatrix}", tex_to_color_map = {r"\begin{bmatrix}1\\0.5\end{bmatrix}": PURPLE, r"\begin{bmatrix}-1\\2\end{bmatrix}": TEAL}).shift(0.5*DOWN + 4*RIGHT).set_stroke(width = 16, color = BLACK, background = True)
        self.play(Write(function))
        self.wait()
        result = MTex(r"\begin{bmatrix}4&2\\2&1\end{bmatrix}\left(\begin{bmatrix}1\\0.5\end{bmatrix}+k\begin{bmatrix}-1\\2\end{bmatrix}\right)=\begin{bmatrix}4&2\\2&1\end{bmatrix}\begin{bmatrix}1\\0.5\end{bmatrix}", tex_to_color_map = {r"\begin{bmatrix}1\\0.5\end{bmatrix}": PURPLE, r"\begin{bmatrix}-1\\2\end{bmatrix}": TEAL}).scale(0.8).next_to(function, DOWN).shift(1.5*LEFT).set_stroke(width = 16, color = BLACK, background = True)
        for i in range(6):
...
class Video_11_3(FrameScene):
    ...
    def construct(self):
        ...
        label_1 = MTex(r"\vec{u}", color = BLUE).next_to(pers(arrow_1.get_end()), UP).fix_in_frame().set_stroke(width = 8, color = BLACK, background = True)
        label_2 = MTex(r"\vec{v}", color = GREEN).next_to(pers(arrow_2.get_end()), RIGHT).fix_in_frame().set_stroke(width = 8, color = BLACK, background = True)
        label_3 = MTex(r"\vec{u}+2\vec{v}", tex_to_color_map = {r"\vec{u}": BLUE, r"\vec{v}": GREEN, r"2": ORANGE}).next_to(pers(arrow_3.get_end()), UR).fix_in_frame().set_stroke(width = 8, color = BLACK, background = True)
        ...
        input_3 = MTex(r"\vec{u}+c\vec{v}", tex_to_color_map = {r"\vec{u}": BLUE, r"\vec{v}": GREEN, r"c": ORANGE}).shift(DOWN + 0.5*LEFT).fix_in_frame()
        ...
        output_3 = MTex(r"\vec{0}+c\vec{0}=\vec{0}", tex_to_color_map = {r"c": ORANGE}).shift(DOWN + 5.5*RIGHT).fix_in_frame()
...
class Video_11_5(FrameScene):
    ...
    def construct(self):
        ...
        operator = np.array([[1/9, 2/9, 2/9], [2/9, 4/9, 4/9], [2/9, 4/9, 4/9]])
        n_0, n_1, n_2 = np.array([1/3, 2/3, 2/3]), np.array([2/3, -2/3, 1/3]), np.array([-2/3, -1/3, 2/3])
        nil = Polygon(5*n_1+5*n_2, 5*n_1-5*n_2, -5*n_1-5*n_2, -5*n_1+5*n_2, stroke_width = 0, fill_opacity = 0.5, fill_color = TEAL, depth_test = True, anti_alias_width = 0).insert_n_curves(12)
        copy_nil = nil.copy().set_fill(color = TEAL_E, opacity = 0.2).scale(1.05, about_point = eye)
        copy_grid = Line(17/9*np.array([1, 2, 2]), 17/9*np.array([-1, -2, -2]), color = YELLOW, depth_test = True, anti_alias_width = 0)

        huaji = TexturedSurface(ParametricSurface(u_range = (-5, 5), v_range = (-5, 5), uv_func = lambda u, v: u*n_1+v*n_2), "huaji.png").set_opacity(1/3)
        huajis = [huaji.copy().shift(i*np.array([1, 2, 2])*2/3).save_state() for i in (-2, -1, 1, 2)]
        self.add(axes, copy_grid, nil, copy_nil, huaji, *huajis).wait()
        self.play(*[FadeOut(mob) for mob in huajis + [nil, copy_nil]])
        self.wait()

        matrix = np.array([n_1, n_2, n_0])
        direction = Rotation.from_matrix(np.array([-n_1, n_2, n_0]).T)
        def line_updater(index: int):
            def util(mob: Line):
                eye = camera.get_orientation().as_rotvec()
                if get_norm(eye) != 0:
                    eye /= get_norm(eye)
                else:
                    eye = OUT
                ortho = cross(eye, n_0)
                mob.put_start_and_end_on(0.2*ortho, -0.2*ortho).shift(index*n_0)
            return util
        lines = [Line(color = YELLOW, depth_test = True).add_updater(line_updater(i)) for i in range(-5, 6)]
        point = Dot().apply_matrix(matrix)
        t = MTex(r"t=-0.00", color = YELLOW).fix_in_frame().shift(3*UP + 5*RIGHT)
        decimal = Value(0.).set_color(YELLOW).fix_in_frame()
        for i in range(5):
            decimal[i].move_to(t[i+2])
        t.remove(t[2])
        self.add(*lines, huaji).play(LaggedStart(*[ShowCreation(line) for line in lines], run_time = 2, lag_ratio = 0.3), GrowFromCenter(point), Write(t))
        left = t[:2]
        self.remove(t).add(left, decimal).wait()
        
        alpha = ValueTracker(0.0)
        def plane_updater(mob: VMobject):
            value = alpha.get_value()
            mob.restore().move_to(value*n_0)
        for mob in [huaji, point]:
            mob.save_state().add_updater(plane_updater)
        def dec_updater(mob: Value):
            value = alpha.get_value()
            mob.set_value(value)
        decimal.add_updater(dec_updater)
        self.play(alpha.animate.set_value(4.), run_time = 3)
        self.wait()
        self.play(alpha.animate.set_value(-3.), run_time = 5)
        self.wait()
        ...
        def color_updater(mob: VMobject):
            mob.set_color(interpolate_color(BLACK, YELLOW, beta.get_value()))
        for mob in [copy_grid, *lines, left, point]:
            mob.add_updater(opacity_updater)
        decimal.add_updater(color_updater)
        self.play(alpha.animate.set_value(0.), beta.animate.set_value(0.), camera.save_state().animate.set_orientation(direction), run_time = 3)
        huaji.clear_updaters()
        self.remove(left, decimal, point).wait()
        ...
        self.add(*lines_h, *lines_v).play(LaggedStart(*[ShowCreation(mob) for mob in lines_h], lag_ratio = 0.3, run_time = 2, group = VGroup()), LaggedStart(*[ShowCreation(mob) for mob in lines_v], lag_ratio = 0.3, run_time = 2, group = VGroup()))
        self.remove(*lines_h, *lines_v).add(grid).wait()

        coordinate = MTex(r"\begin{bmatrix}-0.00\\-0.00\end{bmatrix}", color = TEAL).fix_in_frame().shift(3*UP + 6*RIGHT)
        decimal = VGroup(Value(0.).set_color(TEAL).fix_in_frame(), Value(0.).set_color(TEAL).fix_in_frame())
        for i in range(5):
            decimal[0][i].move_to(coordinate[i+1]), decimal[1][i].move_to(coordinate[i+6])
        coordinate.remove(coordinate[1]).remove(coordinate[5])
        dot = Dot(color = TEAL).fix_in_frame()
        def decimal_updater(mob: VGroup):
            position = dot.get_center()
            mob[0].set_value(position[0]/ratio)
            mob[1].set_value(position[1]/ratio)
        decimal.add_updater(decimal_updater)
        self.play(Write(coordinate), ShowCreation(dot))
        left = VGroup(coordinate[0], coordinate[-1], depth_test = True)
        self.remove(coordinate).add(left, decimal).wait()
        self.play(dot.animate.move_to(ratio*(2*UP + RIGHT)))
        self.wait()
        self.play(dot.animate.move_to(ratio*(3*LEFT + UP)))
        self.wait()
        self.play(dot.animate.move_to(ratio*(DOWN + RIGHT)))
        self.wait()
        decimal.clear_updaters()
        dot.unfix_from_frame().move_to(ratio*(UP + RIGHT)).apply_matrix(matrix)
        self.play(FadeOut(dot), FadeOut(left), FadeOut(decimal), camera.animating(run_time = 3).restore(), beta.animating(run_time = 3).set_value(1))
        for mob in [copy_grid, *lines]:
            mob.clear_updaters()
        self.wait()

        coordinate = MTex(r"\begin{bmatrix}-0.00\\-0.00\\-0.00\end{bmatrix}").fix_in_frame().shift(3*UP + 6*RIGHT)
        coordinate[2:7].set_color(YELLOW)
        coordinate[7:12].set_color(TEAL)
        coordinate[12:17].set_color(TEAL)
        decimal = VGroup(Value(0.).set_color(YELLOW).fix_in_frame(), Value(0.).set_color(TEAL).fix_in_frame(), Value(0.).set_color(TEAL).fix_in_frame())
        for i in range(5):
            decimal[0][i].move_to(coordinate[i+2]), decimal[1][i].move_to(coordinate[i+7]), decimal[2][i].move_to(coordinate[i+12])
        coordinate.remove(coordinate[2]).remove(coordinate[6]).remove(coordinate[10])
        dot = Dot(radius = 0.12).apply_matrix(matrix)
        dot.phase = 0
        def color_updater(mob, dt):
            mob.phase += dt*TAU*2
            mob.set_color(interpolate_color(WHITE, BLUE_E, (np.sin(mob.phase)+1)/2))
        dot.add_updater(color_updater)
        def decimal_updater(mob: VGroup):
            position = dot.get_center()
            mob[0].set_value(np.dot(position, n_0))
            mob[1].set_value(np.dot(position/ratio, n_1))
            mob[2].set_value(np.dot(position/ratio, n_2))
        decimal.add_updater(decimal_updater)
        def follow_updater(mob: VMobject):
            mob.restore().shift(np.dot(dot.get_center(), n_0)*n_0)
        self.play(Write(coordinate), ShowCreation(dot))
        left = VGroup(coordinate[0], coordinate[1], coordinate[-2], coordinate[-1], depth_test = True)
        self.remove(coordinate).add(left, decimal).wait()
        for mob in [huaji, grid]:
            mob.save_state().add_updater(follow_updater)
        ratios = np.array([ratio, ratio, 1])
        self.play(dot.animate.move_to(np.dot(matrix, np.array([1, 1, 1])*ratios)), run_time = 2)
        self.wait()
        self.play(dot.animate.move_to(np.dot(matrix, np.array([-1, 2, 0])*ratios)), run_time = 2)
        self.wait()
        self.play(dot.animate.move_to(np.dot(matrix, np.array([-2, -1, 3])*ratios)), run_time = 2)
        self.wait()
        self.play(dot.animate.move_to(np.dot(matrix, np.array([0, 0, 3])*ratios)), run_time = 2)
        self.wait()
        self.play(dot.animate.move_to(np.dot(matrix, np.array([1, -3, 3])*ratios)), run_time = 2)
        self.wait()
        self.play(dot.animate.move_to(np.dot(matrix, np.array([0, 0, 0])*ratios)), run_time = 2)
        self.wait()
        self.fade_out()
...
class Video_12(FrameScene):
    def construct(self):
        ...
        col_1, col_2, col_3 = MTexText(r"列空间：1维", color = YELLOW).shift(3.25*UP + 4*LEFT), MTexText(r"列空间：2维", color = YELLOW).shift(3.25*UP), MTexText(r"列空间：1维", color = YELLOW).shift(3.25*UP + 4*RIGHT)
        nil_1, nil_2, nil_3 = MTexText(r"零空间：1维", color = TEAL).shift(2.5*UP + 4*LEFT), MTexText(r"零空间：1维", color = TEAL).shift(2.5*UP), MTexText(r"零空间：2维", color = TEAL).shift(2.5*UP + 4*RIGHT)
        sum_1, sum_2, sum_3 = MTex(r"2=1+1").shift(2.5*DOWN + 4*LEFT), MTex(r"3=2+1").shift(2.5*DOWN), MTex(r"3=1+2").shift(2.5*DOWN + 4*RIGHT)
        formula = MTexText(r"总维数=矩阵的秩（列空间维数）+零空间维数", tex_to_color_map = {r"总维数": BLUE, r"矩阵的秩（列空间维数）": YELLOW, r"零空间维数": TEAL}).shift(2.5*DOWN)
...
class Video_13(FrameScene):
    ...
    def construct(self):
        ...
        matrix = MTex(texts[0]+texts[1]+r"="+texts[2], isolate = "=", tex_to_color_map = {texts[0]: ORANGE, texts[1]: GREEN, texts[2]: YELLOW}).scale(0.8).fix_in_frame().set_stroke(width = 8, color = BLACK, background = True).shift(3*RIGHT + 2*UP)
        ...
        solution = MTex(texts[0]+r"="+texts[1]+r"+{t}"+texts[2], tex_to_color_map = {texts[0]: GREEN, texts[1]: YELLOW, texts[2]: TEAL, r"{t}": ORANGE}).scale(0.8).fix_in_frame().set_stroke(width = 8, color = BLACK, background = True).shift(3*RIGHT + 0.5*DOWN)
        solution_1, solution_2 = solution[:15], solution[15:]
        label_1, label_2 = MTex(r"\vec{x}_p", color = YELLOW).next_to(solution[7:16], DOWN).fix_in_frame(), MTex(r"\vec{x}_n", color = TEAL).next_to(solution_2[2:], DOWN).fix_in_frame()
