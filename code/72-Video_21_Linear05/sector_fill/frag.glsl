...
uniform vec3 half_1;
uniform vec3 half_2;
uniform vec3 center_1;
uniform vec3 center_2;
...
void main() {
    if (dot(xyz_coords - center_1, half_1) < 0) discard;
    if (dot(xyz_coords - center_2, half_2) < 0) discard;
    if (color.a == 0) discard;
    // if (length(uv_b2 - mask_center) > mask_radius) discard;
    frag_color = color;
